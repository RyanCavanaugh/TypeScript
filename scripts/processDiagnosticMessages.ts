...
function main(): void {
    ...
    function writeFile(fileName: string, contents: string) {
        ...
        const fileOutputPath = path.join(inputDirectory, fileName);
        fs.writeFileSync(fileOutputPath, contents, "utf-8");
    }

    const inputFilePath = process.argv[2].replace(/\\/g, "/");

    const diagnosticMessagesJson: { [key: string]: DiagnosticDetails } = require(path.resolve(".", inputFilePath));
    const diagnosticMessages: InputDiagnosticMessageTable = new Map(Object.entries(diagnosticMessagesJson));

    const outputFilesDir = path.dirname(inputFilePath);
    const thisFilePathRel = __filename;
